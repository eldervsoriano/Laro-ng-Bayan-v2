...
public class AIPlayer : MonoBehaviour
{
    ...
    private Collider slipperCollider;

    void Awake()
    {
        slipperThrow = GetComponent<SlipperThrow>();
        slipperCollider = GetComponent<Collider>();
    }
    ...
    void Start()
    {
        ...
        // Disable mouse interaction for AI slipper
        if (isAI && slipperCollider != null)
        {
            // Keep collider enabled for physics, but we'll ignore OnMouse events in SlipperThrow
            Debug.Log($"🤖 AI Player {playerNumber} initialized - Throw Acc: {throwAccuracy}, Defense Acc: {defenseAccuracy}");
        }
    }
    ...
    void OnEnable()
    {
        // Reset state when slipper becomes active
        hasThrown = false;
        isThinking = false;
    }
    ...
    {
        if (!isAI) return;
        if (hasThrown) return; // Already threw this turn
        if (isThinking) return; // Already processing throw

        // Check if it's AI's turn to throw
        if (TumbangGameManager.Instance != null &&
            TumbangGameManager.Instance.GetCurrentPlayer() == playerNumber &&
    ...
    private IEnumerator AIThrowSequence()
    {
        ...
        if (TumbangGameManager.Instance.GetCurrentPlayer() != playerNumber)
        {
            Debug.Log($"⚠️ AI Player {playerNumber} - Not my turn anymore, canceling throw");
            isThinking = false;
            yield break;
        }
        ...
        // Execute throw
        if (slipperThrow != null && slipperThrow.rb != null)
        {
            Rigidbody rb = slipperThrow.rb;
            rb.isKinematic = false;
            rb.AddForce(direction * slipperThrow.throwForce, ForceMode.Impulse);
        ...
        {
            Debug.LogError("AI: Missing SlipperThrow component or Rigidbody!");
        }
    ...
    // Called by SlipperThrow.ResetTurn() via TumbangGameManager
    public void ResetAITurn()
    {
        hasThrown = false;
        isThinking = false;
        Debug.Log($"🔄 AI Player {playerNumber} turn reset");
    }
}